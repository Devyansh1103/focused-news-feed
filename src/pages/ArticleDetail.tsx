...
import { featuredArticle, newsArticles, NewsArticle } from '@/data/sampleNews';
...
const ArticleDetail = () => {
  ...
  useEffect(() => {
    ...
    // Fallback: check bundled sample articles to support hero carousel links
    const sampleMatch = newsArticles.find(a => a.id === id);
    if (sampleMatch) {
      setArticle(sampleMatch);
      return;
    }

    // If not found in locally loaded list, fetch directly by id to avoid 404s
  ...
  // Generate an expanded summary (15–20 sentences) when article loads
  useEffect(() => {
    const run = async () => {
      if (!article) return;
      try {
        const baseText = ('content' in article && article.content) ? article.content : (article.summary || '');
        if (!baseText) return;
        const { data, error } = await supabase.functions.invoke('summarize-article', {
          body: {
            articleContent: baseText,
            // Request a long summary; backend interprets as words
            maxLength: 700
          }
        });
        if (!error && data?.summary) {
          setGeneratedSummary(String(data.summary));
        }
      } catch (_) {
        // Best-effort; ignore errors and keep existing summary
      }
    };
    run();
  }, [article]);
          ...
          {(generatedSummary || article.summary) && (
            <div className="mb-8">
              <div className="text-lg text-muted-foreground leading-relaxed space-y-4">
                {(generatedSummary || article.summary)?.split('\n\n').map((paragraph, index) => (
