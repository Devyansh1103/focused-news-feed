    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: `You are a professional news summarizer. Produce a clear, well-structured summary of the article in 15–20 sentences, grouped into short paragraphs. Stay factual, avoid opinions, and cover: who/what/when/where/why/impact/what's next. Keep under ${maxLength} words total.`
          },
          {
            role: 'user',
            content: `Summarize this article thoroughly (15–20 sentences):\n\n${articleContent}`
          }
        ],
        max_tokens: Math.min(maxLength * 2, 1200),
        temperature: 0.3,
      }),
    });
